...
import { FormSchema, FormField, FormFieldType, FormsIndexEntry } from '@/types/form';
...
export const FormBuilder: React.FC<FormBuilderProps> = ({ onSaved, onBack }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const manifestBlobId = await uploadToWalrus([], 5);

      toast.loading("Uploading schema to Walrus...", { id: 'save' });
      const schemaWithManifest = { ...schema, manifestBlobId };
      const schemaBlobId = await uploadToWalrus(schemaWithManifest, 5);
      ...
      // We don't have formObjectId yet — will be filled after tx success.
      // Upload the index with a placeholder; we'll update after getting the object ID.
      ...
      const newRegistryBlobId = await uploadToWalrus(newIndex, 5);

      // Generate ephemeral keypair to embed in the share URL so submitters can sign transactions
      ...
      // Fetch the new Form object ID from the transaction effects
      toast.loading("Confirming on-chain...", { id: 'save' });
      const formObjectId = await getFormObjectIdFromDigest(txDigest);

      // Now re-upload the forms index with the real formObjectId
      if (formObjectId) {
        ...
        const finalRegistryBlobId = await uploadToWalrus(finalIndex, 5);

        // Update registry on Sui with the final blob ID
